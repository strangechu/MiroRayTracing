...
Ray
Camera::eyeRay(int x, int y, int imageWidth, int imageHeight, bool dof)
{
	...
	if (dof)
	{
		float rw = ((rand() % RAND_MAX) / RAND_MAX) * 2.0f - 1.0f;
		float rh = ((rand() % RAND_MAX) / RAND_MAX) * 2.0f - 1.0f;
		Vector3 sampledEye = m_eye + Vector3(rw, rh, 0.0f);
		Vector3 P = m_eye - pixelPos * 10.0f;
		return Ray(sampledEye, (P - sampledEye).normalize());
	}
