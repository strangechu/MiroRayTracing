...
makeReflectScene()
{
	...
	// teapot 1
	material = new Lambert(Vector3(1.0f));
	...
	xform.setIdentity();
	xform *= scale(0.5, 0.5, 0.5);
	xform *= translate(3, 0, 3);
	//xform *= rotate(25, .3, .1, .6);
	teapot->load("teapot.obj", xform);
	material->setColor(Vector3(0.0f, 1.0f, 0.0f));
	addMeshTrianglesToScene(teapot, material);
	...
	//xform *= rotate(25, .3, .1, .6);
	teapot->load("teapot.obj", xform);
	material->setColor(Vector3(0.0f, 0.0f, 1.0f));
	addMeshTrianglesToScene(teapot, material);
	...
	material->setColor(Vector3(0.5f, 0.5f, 0.0f));
	g_scene->addObject(t);

	// create the far plane triangle
/*
	floor = new TriangleMesh;
	floor->createSingleTriangle();
	floor->setV1(Vector3( -5,  0, 1));
	floor->setV2(Vector3(  5,  0, 1));
	floor->setV3(Vector3(  0, 10, 1));
	floor->setN1(Vector3(0, 0, 1));
	floor->setN2(Vector3(0, 0, 1));
	floor->setN3(Vector3(0, 0, 1));
...
makeTeapotScene()
{
	...
	Material* floor_material = new Lambert(Vector3(1.0f));
	Triangle* t = new Triangle;
	t->setIndex(0);
	t->setMesh(floor);
	t->setMaterial(floor_material); 
	g_scene->addObject(t);
